...
class BacktestEngine:
    def __init__(self, symbol, trade_qty, start="", end=""):
        ...
        self.market_data_source = MarketDataSource(
            symbol, tick_event_handler=self.on_tick_event, start=start, end=end
        )
    ...
    def start(self, **kwargs):
        ...
        self.strategy = MeanRevertingStrategy(
            self.symbol, self.trade_qty, send_order_event_handler=self.on_order_received, **kwargs
        )
    ...
    def on_order_filled(self, symbol, qty, is_buy, filled_price, timestamp):
        ...
        print(
            timestamp.date(), "FILLED", "BUY" if is_buy else "SELL", qty, symbol, "at", filled_price
        )
...
if __name__ == "__main__":
    ...
    engine = BacktestEngine("WIKI/AAPL", 1, start="2015-01-01", end="2017-12-31")
    engine.start(lookback_intervals=20, buy_threshold=-1.5, sell_threshold=1.5)

    engine.df_rpnl.plot(figsize=(12, 8))
    plt.savefig("images/ch09-im01.png", dpi=300, bboxinches="tight")
    ...
    for i, (buy_threshold, sell_threshold) in enumerate(THRESHOLDS):
        engine.start(
            lookback_intervals=20, buy_threshold=buy_threshold, sell_threshold=sell_threshold
        )
        df_rpnls = engine.df_rpnl
        ax = axes[i // 2, i % 2]
        ax.set_title("B/S thresholds:(%s,%s)" % (buy_threshold, sell_threshold))
        df_rpnls.plot(ax=ax)
    plt.savefig("images/ch09-im02.png", dpi=300, bboxinches="tight")
